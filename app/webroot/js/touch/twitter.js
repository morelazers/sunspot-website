	   ...
	   //alert(feeds);
        ...
        for (var i=0; i<feeds.length; i++) {
			...
			/*console.log(feeds[i]);*/
			 ...
			 if (((showretweets == true) || ((isaretweet == false) && (showretweets == false))) && ((showdirecttweets == true) || ((showdirecttweets == false) && (isdirect == false)))) { 
			 	/*console.log(feeds[i].text.length);*/
				if ((feeds[i].text.length > 1) && (displayCounter <= displaylimit)) {             
					...
					/*console.log("THITHIEWHRK");*/
								 
					feedHTML += '<div class="twitter-article" id="tw'+displayCounter+'">'; 										                 
					feedHTML += '<div class="twitter-text"><a class="twitter-date" href="https://twitter.com/'+tweetusername+'/status/'+tweetid+'" target="_blank">'+relative_time(feeds[i].created_at)+': </a>"'+status+'"</br><a class="twitter-username" href="https://twitter.com/'+tweetusername+'" target="_blank">@'+tweetusername+'</a></span><span class="tweet-time">';
		...
		if (showtweetactions == true) {				
			$('.twitter-article').hover(function(){
				...
				$(this).find('#twitter-actions').animate({'opacity':1, 'margin-top':-20},120, function(){
		...
		function animatetweets() {	
			...
			for (var i=starttweet; i<displayCounter; i++) {				
				$('#tw'+i).css({'display': 'block', 'opacity':0});
			}			
			fadetweet();
			function fadetweet(){
